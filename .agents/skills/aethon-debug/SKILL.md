...
```bash
.agents/skills/aethon-debug/scripts/debug-eval.sh 'return 1 + 1'
.agents/skills/aethon-debug/scripts/debug-eval.sh 'return window.__AETHON_STATE__().model'
.agents/skills/aethon-debug/scripts/debug-eval.sh --file snippet.js   # multi-line JS
.agents/skills/aethon-debug/scripts/debug-screenshot.sh
```
...
Or via the slash-command form, which reads from `.agents/skills/aethon-debug/scripts/`:
...
```bash
.agents/skills/aethon-debug/scripts/debug-eval.sh 'window.focus(); return "focused"'
```
...
```bash
.agents/skills/aethon-debug/scripts/debug-eval.sh <<'JS'
...
```bash
.agents/skills/aethon-debug/scripts/debug-wait.sh
```
...
```bash
.agents/skills/aethon-debug/scripts/debug-eval.sh 'return window.__AETHON_STATE__()'
.agents/skills/aethon-debug/scripts/debug-eval.sh 'return window.__AETHON_STATE__().messages'
.agents/skills/aethon-debug/scripts/debug-eval.sh 'return window.__AETHON_STATE__().terminal'
```
...
.agents/skills/aethon-debug/scripts/debug-logs.sh

# Filter by scope (matches the scope column in each line)
.agents/skills/aethon-debug/scripts/debug-logs.sh --grep ext-loader

# Live tail
.agents/skills/aethon-debug/scripts/debug-logs.sh --follow

# Pick a source: rust | bridge | all (default)
.agents/skills/aethon-debug/scripts/debug-logs.sh --source bridge

# Look at a specific day (YYYY-MM-DD) — useful after a rotation
.agents/skills/aethon-debug/scripts/debug-logs.sh --since 2026-05-01

# More lines than the default 50
.agents/skills/aethon-debug/scripts/debug-logs.sh --lines 500
```
...
```bash
.agents/skills/aethon-debug/scripts/debug-eval.sh 'USER_JS_HERE'
```
...
```bash
.agents/skills/aethon-debug/scripts/debug-eval.sh 'window.aethon.resetLayout(); return "reset"'
```
...
```bash
.agents/skills/aethon-debug/scripts/debug-invoke.sh shell_list_shareable
.agents/skills/aethon-debug/scripts/debug-invoke.sh devshell_status '{"args":{"root":"'"$(pwd)"'"}}'
.agents/skills/aethon-debug/scripts/debug-invoke.sh debug_shell_snapshot '{"tabId":"uat-1","tailBytes":4096}'
```

### Shell-tab driving (PTY user-equivalent)

```bash
TAB=$(.agents/skills/aethon-debug/scripts/debug-shell-spawn.sh)              # open in active project
TAB=$(.agents/skills/aethon-debug/scripts/debug-shell-spawn.sh /path/repo)   # explicit cwd
.agents/skills/aethon-debug/scripts/debug-shell-write.sh "$TAB" 'env | sort | head'
.agents/skills/aethon-debug/scripts/debug-shell-read.sh   "$TAB"             # JSON snapshot (cwd, command, share, tail)
.agents/skills/aethon-debug/scripts/debug-shell-read.sh   "$TAB" 8192 raw    # just the tail bytes
.agents/skills/aethon-debug/scripts/debug-shell-close.sh  "$TAB"
```
...
```bash
.agents/skills/aethon-debug/scripts/debug-devshell.sh status   # active project's cache state
.agents/skills/aethon-debug/scripts/debug-devshell.sh env      # resolved env map
.agents/skills/aethon-debug/scripts/debug-devshell.sh refresh  # invalidate + re-resolve
.agents/skills/aethon-debug/scripts/debug-devshell.sh status /Users/.../some-project   # explicit root
```
...
```bash
TAB=$(.agents/skills/aethon-debug/scripts/debug-agent-new-tab.sh /path/to/project)
.agents/skills/aethon-debug/scripts/debug-chat-send.sh "$TAB" 'run env | grep PATH via your bash tool'
.agents/skills/aethon-debug/scripts/debug-chat-wait.sh "$TAB" 90    # block until the response lands
```
