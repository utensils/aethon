...
| `extensions.md` | Authoring extensions: directory layout under `~/.aethon/extensions/`, project-local `.aethon/extensions/`, npm-distributed skill packages under `~/.aethon/skills/node_modules/`, and 6 worked examples (theme, sidebar section, live data via `setState`, event handler that runs an LLM turn, layout patch, custom A2UI component). |
