...
| `components.md` | A2UI primitive components (`text`, `heading`, `paragraph`, `card`, `button`, `container`, `divider`, `code`, `image`, `icon`, `text-input`, `date-picker`, `checkbox`, `select`, `slider`, `list`, `table`, `form`, `form-field`) and extension-provided composites (`layout`, `sidebar`, `tab-strip`, `chat-history`, `chat-input`, `status-bar`, `terminal`, `main-canvas`). Includes prop schemas and the `$ref` JSON-Pointer data-binding contract. |
