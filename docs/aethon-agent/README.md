...
| File                | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `api.md`            | `globalThis.aethon` runtime API and the agent-visible tools: `setState`, `setLayout`, `patchLayout`, `canvas.*`, `editor.openFile`, `openFileInEditor`, `registerComponent`, `registerSidebarSection`, `registerTheme`, `registerHighlightGrammar`, `onEvent`, plus the introspection helpers (`getFrontendState`, `listExtensions`, `listComponents`, `listThemes`, `getLayout`, `getRuntimeSnapshot`).                                                            |
| `components.md`     | A2UI primitive components (`text`, `heading`, `paragraph`, `card`, `button`, `container`, `divider`, `code`, `image`, `icon`, `text-input`, `date-picker`, `checkbox`, `select`, `slider`, `list`, `table`, `form`, `form-field`, `for-each`) and extension-provided composites (`layout`, `sidebar`, `tab-strip`, `chat-history`, `chat-input`, `status-bar`, `terminal`, `main-canvas`). Includes prop schemas and the `$ref` JSON-Pointer data-binding contract. |
| `extensions.md`     | Authoring extensions: directory layout under `~/.aethon/extensions/`, project-local `.aethon/extensions/`, npm-distributed extension packages under `~/.aethon/extensions/node_modules/`, and worked examples (theme, sidebar section, live data via `setState`, event handler that runs an LLM turn, layout patch, custom A2UI component).                                                                                                                         |
| `project-config.md` | Project-local `.aethon` config files, including `.aethon/issues.toml` issue-to-agent prompt templates.                                                                                                                                                                                                                                                                                                                                                              |
