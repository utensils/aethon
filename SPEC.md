...
- [ ] Extension hot-reload — currently requires the agent process to restart (touch `agent/main.ts` or send a re-import command); pi has `/reload` for its own discovery path but Aethon's loader doesn't subscribe to it yet
- [~] Discovery wired for `~/.aethon/extensions/` (above). Project-local `.aethon/extensions/` and explicit additional paths from `~/.aethon/config.toml` are not yet wired
...
- [ ] Native menus (currently default macOS chrome only — no app-specific items beyond the standard Edit / Window menus the framework provides)
