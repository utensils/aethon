...
- [x] Extension-registered components are interactive — `aethon.onEvent({templateRootType, componentType, descendantId, eventType, surfaceId, windowId}, handler)` runs handlers when an A2UI control inside an extension template fires an event. Handlers can call `setState` / `registerComponent` to drive UI without an LLM round-trip; events from native canvas windows also receive `ctx.window` helpers scoped to that OS window. Renderer threads `templateRootType` through descendant dispatches and the bridge extracts `descendantId` from the host-prefixed componentId. Demo at `examples/pi-extensions/aethon-counter.ts`.
...
- [x] **Native A2UI canvas windows** — `globalThis.aethon.windows.openCanvas({ id?, title?, components?, state?, width?, height?, x?, y?, focus?, restoreOnLaunch? })` creates a normal native OS window at `index.html?surface=canvas-window&id=<id>` using label `aethon-canvas-<id>`. V1 supports `kind: "canvas"` records only, persists restorable open windows to `~/.aethon/native-windows.json`, reuses window geometry persistence for dynamic labels, and removes records on close. The lightweight canvas-window React root hydrates the record from Rust, renders `A2UIRenderer` in bare controlled mode, persists debounced local state, and replays extension templates, `aethon.frontendEntry` React modules, themes, and highlight grammars without booting the main bridge hook. Bridge-side tools mirror the namespace (`openA2uiCanvasWindow`, `emitA2uiWindowCanvas`, `setA2uiWindowState`, etc.), `RuntimeSnapshot.nativeWindows` lists summaries, and the prompt now prefers native canvas windows for exploratory/custom UI so agents don't modify main chrome unless asked.
- [x] Extension hot-reload — bridge file watcher now runs in dev AND release, watches `~/.aethon/extensions/`, `~/.aethon/extensions/node_modules/`, and `~/.pi/agent/extensions/` (when present), plus `<project>/agent/` in dev only. Trailing-edge debounce via a single dedicated worker thread (mpsc channel + `recv_timeout`) collapses npm-install bursts into one settled reload request. `~/.aethon/extensions` is pre-created on boot so first-install Create events fire without a manual restart.
...
- `windows.{openCanvas, list, focus, close, setTitle, emitCanvas, appendCanvas, patchCanvas, clearCanvas, setState}` — Native OS windows hosting bare A2UI canvas payloads with window-local state. Events include `surfaceId` / `windowId`, and handlers invoked from these windows receive `ctx.window`.
...
3. **Multiple canvases** — Yes. Tabs provide one canvas per agent session, and
   native canvas windows provide separate restorable OS-window canvases for
   exploratory/custom UI that should not replace the main workspace chrome.
