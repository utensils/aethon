...
- [~] Aethon-side extensions also work via `~/.aethon/extensions/*.{ts,js}` exporting `register(api)` (same API surface). Bridge retains state as a tree and replays on `ready`; frontend hydrates templates into the SkillRegistry and the renderer expands them inline with host-prefixed ids. `registerPanel` / `registerTheme` and a deeper pi-extension ctx integration (so handlers can also call into pi's session/tools) are follow-ups.
