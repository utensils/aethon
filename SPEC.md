...
Last reviewed: 2026-04-27 (M5 complete — only release work remains: Nix overlay, first public release). Items shipped this pass include mutation feedback, frontend-state mirror, empty-state composite, for-each, chrome props, theme directory, layout structure snapshot, registerable slash commands + keybindings + menu items + event routes, compositional sidebar items + terminal subscription, primitive expansion (heading/paragraph/divider/checkbox/select/slider/list/table), multi-tab restore via empty-state, layout catalogue + sidebar opt-in, dev-port auto-increment, handler attribution race, config dead options, and the **layout-slot contract** (`slots.json` + canonical area names + optional `slotMap`).
