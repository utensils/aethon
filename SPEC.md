...
- [x] **Drag-and-drop file paths into composer** — Tauri 2's `onDragDropEvent` delivers absolute paths directly (HTML5 dataTransfer is sandboxed). On drop into an agent tab, paths are appended to `/draft` as `@<absolute-path>` tokens (multiple → space-separated). Shell tabs ignore drops in v1; image drops + Cmd/Ctrl+V image paste deferred to a follow-up.
...
- [x] **Tool-card elapsed-time + long-running warning** — `tool-card` builtin component (replaces plain `card` for tool calls). Shows live `running… 3.2s` clock at 4 Hz while `startedAt` is set and `endedAt` isn't; on end shows `completed in 12.4s` (or `failed in …` on error). At 30 s, the title shifts amber and the card surfaces a "Long-running command — press ⌘. to stop" hint. The bridge sends `startedAt` on `tool_execution_start` and `endedAt` on `tool_execution_end`; the React component owns the timer locally so the bridge doesn't have to push tick updates.
