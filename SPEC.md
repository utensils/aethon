...
- [ ] **Per-tab share mode** — four-value enum: `private` (default) | `read` (agent reads scrollback, no writes) | `read-write` (agent writes with per-write user confirmation) | `read-write-trusted` (agent writes without confirmation; explicit opt-in for "drive my shell" workflows). Settable via clickable badge in the terminal status line (cycles through values), right-click menu, or palette ("Shell sharing → ..."). The default for new shell tabs is configurable via `[shell] default_share_mode`; allowed config values are the same four. Mode is per-tab and opt-in; the badge is always visible — no hidden state.
- [ ] **`aethon.shells.{list,read,write}` API + agent tools** — `listShells()` returns shareable-tab metadata only (`shareMode !== "private"`); `readShell({tabId, lines?, since?})` returns recent scrollback (default last 200 lines, max 5000); `writeShell({tabId, text})` injects keystrokes (gated to `read-write` and `read-write-trusted`). Tools register through the existing pi extension system. `ctx.shells` namespace surfaces the same on event handler ctx.
- [ ] **Per-write user confirmation in `read-write` mode** — each `writeShell` call prompts the user inline (Allow / Allow-once / Deny). `read-write-trusted` skips the prompt — that's the only behavioral difference between the two writable modes. The confirmation UI is an A2UI overlay (no new primitive).
- [ ] **Privacy guardrail: mode flips don't expose old scrollback** — switching from `private` → any shareable mode exposes only content from the flip onward. Bridge tracks line offset at flip time and uses it as the floor for `readShell`. Sharing-mode changes emit `shell-share-change {tabId, mode}` so `RuntimeSnapshot` reflects the current mode.
