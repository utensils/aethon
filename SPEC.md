...
- [ ] **`getLayout()` returns the active rendered layout** — currently `_getLayout()` in `agent/main.ts` returns `extensionLayout ?? null`, NOT the boot layout from `src/skills/default-layout/layout.a2ui.json`. Extensions inspecting at register-time see `null` and bail out (verified failure mode for a real extension). Fix: frontend sends the canonical boot layout to the bridge on first connect (new `boot_layout` outbound message), bridge stores it as `bootLayout`, and `_getLayout()` returns `extensionLayout ?? bootLayout`. Bonus: `getLayout()` can also overlay `pendingLayoutPatches` so the result reflects what's actually rendered, not what was originally booted.
- [ ] **Bridge-readable frontend state** — bridge currently sees only its own writes (`extensionStateTree`, `perTabExtState`) and bootstrap-time `cachedModels`/`tabs`. Frontend-populated keys are invisible: `/sidebar/models` (filled by `ready` event), `/sidebar/themes`, `/messages`, `/draft`, `/connection`, `/status`. Extensions can't query "what models are available?" or "is the user typing?" without scraping. Fix: pick a small allowlist of mirrored keys, frontend sends `frontend_state_patch` events whenever they change, bridge stores in a `frontendState` map and exposes via `aethon.getFrontendState(path)` plus folds into `getRuntimeSnapshot().uiState`.
- [ ] **Mutation feedback channel** — `setState` / `setLayout` / `patchLayout` / `registerComponent` / `registerTheme` / `registerSidebarSection` are sync `void`. No way for the agent to know if a write was rejected (bad payload), silently no-op'd (path doesn't exist), or clobbered an existing registration. Fix: assign every mutation an auto-generated ID, frontend acknowledges via `mutation_ack { id, success, error? }`, bridge resolves a `Promise` so the API becomes `await aethon.setState(...)` returning `{ok: bool, error?: string}`. Failures additionally surface as a `notice` chat bubble so the next agent turn has the error in context. Sync forms remain available for the fire-and-forget cases.
...
- [ ] **Registerable keyboard shortcuts** — `Cmd+T` / `Cmd+]` / `Cmd+[` / `Cmd+W` / `Cmd+\`` are bound directly to `document.keydown` in `App.tsx:662-710`. Extensions can't override or add new combos. Fix: store bindings in `/keybindings` state as `[{combo, action, scope?, sourceId?}]`, dispatch via a single state-driven listener, expose `aethon.registerKeybinding({combo, action})` and `aethon.unregisterKeybinding(combo)`. Built-ins seed the array on boot from default-layout but can be overridden / removed.
...
- [ ] **Closing the last tab shows an empty-state view, not a forced-empty default tab** — today `closeTab` is a no-op when the active tab is the only one (and the "default" tab can't be closed at all). The user can never get to a "no conversations open" state. Fix: allow closing every tab. When the tab list is empty, the layout swaps in an `empty-state` composite (welcome card, "New Tab" button, recent-sessions list, quick-start tips). The empty state ships **as part of `default-layout`**, not as hardcoded React in `App.tsx` — it's an A2UI composite registered by the default-layout skill, addressable via `$ref` and replaceable by an extension that wants a different empty-state UI. New-tab gestures (`Cmd+T`, the `+` button, the system-tray "New Tab", the menu) work from the empty state; the `tabs` array re-populates when a new tab is created. The bridge gracefully handles an empty `tabs` map (no `currentAgentTabId`, no per-turn ALS, `ensureTab` re-creates "default" or whatever tab the frontend opens next).

#### Documentation + agent guardrails

- [ ] **System prompt warns about array iteration limit** — until `for-each` lands, the prompt should explicitly tell the agent "dynamic lists require regenerating the subtree on each mutation via `patchLayout`; A2UI templates do not iterate arrays into children." Saves a debugging round-trip.
- [ ] **System prompt covers the mutation feedback contract** — once the ack channel exists, document that `aethon.setState(...)` returns a Promise; sync use is fire-and-forget; failures appear as system bubbles.
...
- [x] Native menus — full bar built with `tauri::menu::MenuBuilder` replaces Tauri's auto-generated default. Standard NS items (Quit, Hide, Cut/Copy/Paste, Minimize, …) come from `PredefinedMenuItem` so they get free native behavior; app-specific items emit a `menu` Tauri event whose payload routes into the same React dispatcher Cmd+T / Cmd+] / Cmd+W use, so menu and shortcuts can never drift. Layout: Aethon (macOS) / File (New Tab, Close Tab) / Edit / View (Toggle Terminal, Clear Chat, Stop Prompt, Check for Updates… on non-macOS) / Tabs / Window. `Cmd+W` is reserved for `close_tab`; the predefined `close_window` is omitted so macOS doesn't route it to the wrong action.
