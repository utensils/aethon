...
- [x] **Per-tab share mode** — four-value enum: `private` (default) | `read` (agent reads scrollback, no writes) | `read-write` (agent writes with per-write user confirmation) | `read-write-trusted` (agent writes without confirmation). Settable via clickable badge in the shell-tab status line (cycles through values, color-coded for at-a-glance privacy state). The default for new shell tabs is configurable via `[shell] default_share_mode`; unknown values fall back to `"private"` so a typo can't widen access. Mode is per-tab; the badge is always visible — no hidden state. Right-click menu / palette entries deferred to a follow-up.
- [x] **`aethon.shells.{list,read}` API** — `aethon.shells.list()` returns shareable-tab metadata only (`shareMode !== "private"`); `aethon.shells.read({tabId, sinceTotal?, maxBytes?})` returns recent scrollback bytes (default 8 KiB tail, hard cap 64 KiB) clamped to ≥ the privacy floor. Round-trips through the existing mutation-ack channel as `shell_query`; `MutationResult` extended with `data?: unknown`. `aethon.shells.write` + pi-tool registration (`listShells`/`readShell`/`writeShell`) deferred to P2.2 alongside the per-write confirmation overlay.
- [ ] **Per-write user confirmation in `read-write` mode** — each `writeShell` call prompts the user inline (Allow / Allow-once / Deny). `read-write-trusted` skips the prompt — that's the only behavioral difference between the two writable modes. The confirmation UI is an A2UI overlay (no new primitive). **Deferred to P2.2.**
...
- [x] **`[shell] default_share_mode`** (default `"private"`, allowed: `private | read | read-write | read-write-trusted`, anything else clamped to `private`) — surfaced from `~/.aethon/config.toml` and seeded into `newShellTab` so new tabs open at the configured mode. Rust `shell_set_share_mode` is invoked after `shell_open` to keep frontend and Rust state in lockstep.
- [ ] **Remaining `[shell]` keys** — `default_command` (default `$SHELL`), `default_args` (default `["-il"]` Unix, `[]` Windows), `inherit_env` (default true), `prompt_before_close` (default true), `auto_restart_agent` (default true). Subsumes the existing `[shell]` login-PATH override backlog item. **Pending.**
