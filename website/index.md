...
features:
  - icon: { src: /icons/canvas.svg, width: 26, height: 26 }
  ...
  - icon: { src: /icons/tabs.svg, width: 26, height: 26 }
    title: Tabs and PTY shells
    details: Top-strip agent tabs each own a pi conversation, model, and bash buffer. The bottom panel hosts xterm-powered PTY shells for vim, htop, and fzf, fully theme-aware.
    link: /guide/agent-tabs
    linkText: Tab and shell guide
  - icon: { src: /icons/shield.svg, width: 26, height: 26 }
  ...
  - icon: { src: /icons/palette.svg, width: 26, height: 26 }
    title: Themes and extensions
    details: Seven themes ship in the box. Ember and Paper lead, with Æther, Brink, Daylight, Mist, and Nocturne. Drop a `.ts` into `~/.aethon/extensions/` to add slash commands, components, or layouts. Project-local extensions auto-discover.
    link: /guide/themes
...
<p style="max-width: 1180px; margin: 3rem auto 0;">
  <img src="/aethon-app-screenshot.png" alt="Aethon workstation showing projects, the project dashboard, and file explorer" style="width: 100%; border: 1px solid var(--vp-c-divider); border-radius: 12px; box-shadow: 0 12px 32px color-mix(in srgb, var(--vp-c-black) 22%, transparent);" />
</p>
