...
_shell sub-tabs_ instead — see [Shells & share modes](/guide/shells-and-share-modes).
...
- **Model** — the model picked for _this_ tab. Defaults to `[agent] model`.
...
To change the _default_ model for new tabs, edit `[agent] model` in
...
it arrives. Bash output is _also_ mirrored to the "Agent bash"
...
While a turn is running, Aethon shows a short activity line in the chat and
mirrors the same state in the footer. It summarizes the active work ("Reading
directory contents", "Running checks", "Writing response") instead of flashing
raw tool calls. Completed file edits render as an "Edited files" disclosure;
collapse it to hide the whole file list, or open individual/all changed files
from the card.

## Turn actions

Every completed turn can be forked into a new tab from that point in the
conversation. Forking is local and should open quickly even while another
agent tab is busy. Rollback is only shown where it can sensibly truncate the
current conversation; fork remains available anywhere a branch point exists.
