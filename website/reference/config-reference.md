...
| `thinking_level` | `"off" \| "minimal" \| "low" \| "medium" \| "high" \| "xhigh" \| "max" \| "ultra"` | unset | Default reasoning effort for new agent sessions on models that expose it. Options are model-specific: GPT-6 Astra and GPT-5.6 Sol/Terra support through Ultra, Luna through Max, and older models retain their existing sets. Unknown values are ignored (treated as unset). Per-session overridable via the model-picker reasoning selector. |
