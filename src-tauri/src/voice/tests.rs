// Integration-scoped suite kept together rather than split across the
// submodules it covers. The tests share heavyweight fixtures (temp-dir
// settings DBs, WAV decoding, Whisper forward probes) and exercise
...
async fn start_platform_recording_prepares_permission_on_user_action() {
    let (_db_dir, db_path) = test_db_path();
    let platform_engine = Arc::new(FakePlatformSpeechEngine::needs_speech_permission_then_ready());
...
impl Lfm2Backend for FakeLfm2Backend {
    ...
    async fn asr(&self, _audio: CapturedAudio, cancel: Arc<AtomicBool>) -> Result<String, String> {
        self.calls.fetch_add(1, Ordering::Relaxed);
    ...
    async fn tts(&self, _text: String, cancel: Arc<AtomicBool>) -> Result<CapturedAudio, String> {
        self.calls.fetch_add(1, Ordering::Relaxed);
...
impl FakeLfm2Backend {
    ...
    async fn wait_for_cancel(&self, cancel: &Arc<AtomicBool>, message: &str) -> Result<(), String> {
...
async fn cancel_during_lfm2_transcription_returns_early() {
    ...
    let stop_registry = Arc::clone(&registry);
    let stop_handle = tokio::spawn(async move { stop_registry.stop_and_transcribe(LFM2_ID).await });
    tokio::time::sleep(Duration::from_millis(50)).await;
...
async fn ignored_real_lfm2_tts_synthesizes_audio() {
    ...
    if let Some(out) = std::env::var_os("AETHON_TTS_OUT") {
        ...
        let mut writer = hound::WavWriter::create(PathBuf::from(out), spec).expect("wav writer");
...
async fn ignored_real_playback_plays_tone() {
    ...
    let samples: Vec<f32> = (0..count)
        .map(|i| 0.2 * (2.0 * std::f32::consts::PI * freq * i as f32 / sample_rate as f32).sin())
        .collect();
