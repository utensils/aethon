...
fn write_config(config: serde_json::Value, app: AppHandle) -> Result<(), String> {
    ...
    // Pull each known key, with type coercion + default fallback. This
    // is verbose but keeps the surface explicit — no surprise keys leak
    // through from a malicious payload.
    ...
    // Build the TOML output by hand. Cleaner than round-tripping through
    // the toml crate's Value tree for this small surface, and gives us
    // explicit control over section order + leading comment.
    let mut out = String::new();
    out.push_str("# ~/.aethon/config.toml — generated by Aethon Settings.\n");
    out.push_str("# Hand-edits to keys not exposed in the Settings UI are preserved\n");
    out.push_str("# only if you avoid clicking Save in the panel; otherwise this file\n");
    out.push_str("# is rewritten with the canonical key set.\n\n");
    ...
    if let Some(n) = font_size {
        out.push_str(&format!("font_size = {n}\n"));
    }
    if let Some(b) = restore_tabs {
        out.push_str(&format!("restore_tabs = {b}\n"));
    }
    if let Some(b) = notify_on_completion {
        out.push_str(&format!("notify_on_completion = {b}\n"));
    }
    if let Some(n) = notify_min_duration {
        out.push_str(&format!("notify_min_duration_seconds = {n}\n"));
    }
    ...
    out.push_str(&format!(
        "default_share_mode = {}\n",
        toml_string(default_share_mode)
    ));
    if let Some(b) = auto_restart_agent {
        out.push_str(&format!("auto_restart_agent = {b}\n"));
    }
...
/// TOML string-literal escape. The `toml` crate's serializer would
/// handle this for us, but we're emitting by hand for the section
/// ordering — this helper covers the cases we care about. Keep it
/// simple: backslash-escape backslashes + quotes, wrap in double
/// quotes. Newlines / control chars don't appear in any value we
/// write here (theme ids are slugs, model ids match
/// `[a-z0-9/_.-]+`).
